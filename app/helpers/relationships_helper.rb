module RelationshipsHelper
	def relationship_button_for(entity, connected_with, relationship = nil)
		if !relationship.blank? or entity.connected_to?(connected_with)
			if !relationship.blank?
				return link_to '<i class="icon icon-remove"></i> Remove from Group'.html_safe, reject_relationship_path(:id => relationship.id), :method => :destroy, :class => 'btn', :confirm => "Are you sure you want to remove #{connected_with.name} as a connection?"
			else
				return link_to 'View Profile', connected_with, :class => 'btn btn-large'
			end
