class StartupsController < ApplicationController
  ...
  def search
    ...
    if @search[:search_type] == 'location'
      ...
      @results = User.geocoded.geo_scope(:origin => origin).active.where(:startup_id => @startups_by_id.keys).group(:startup_id).order(:distance).paginate(:page => params[:page] || 1, :per_page => 10) 
